...
TO BE IMPLEMENTED/RECTIFIED -
    -> Sending n>9 messages (Sending buffer limit = 10)
    -> Garbage cleaning
    -> Receiving messages in window size but not the just next message
    -> Handling duplicate ACKs
    -> Sending Duplicate ACKs
    -> Updated size sending
    -> Sephamores
*/
            ...
            if (addr[i].isfree == 0 && FD_ISSET(addr[i].sockfd, &fds)) {
                ...
                if (buff[0] == '0') {
                    if (addr[i].rwnd.last_message == seq) {   // Old message
                        ...
                        bzero(addr[i].receive_buffer[0], sizeof(addr[i].receive_buffer[0]));
                        
                        addr[i].rwnd.window_size--;
                        int index = (addr[i].rwnd.rb_end + 1)%10;
                        addr[i].rwnd.rb_end = index;
                        strcpy(addr[i].receive_buffer[index], &buff[4]);
                        printf("Received - %s\n", addr[i].receive_buffer[index]);
                        bzero(&buff[4], sizeof(buff)-4);
                        buff[0] = '1';
                        buff[4] = '0'+addr[i].rwnd.window_size;
                    ...
                    if (seq == addr[i].swnd.seq) {
                        addr[i].swnd.receive_size = max(buff[4]-'0', addr[i].swnd.receive_size);
                        addr[i].swnd.seq++;
                        addr[i].swnd.sb_start++;
                        addr[i].swnd.sb_start %= 10;
                    }

                    bzero(&addr[i].send_buffer[0], sizeof(addr[i].send_buffer[0]));
                }
            } else if (addr[i].isfree == 0 && 0) { // check_send_timeout(&addr[i].last_send, 20*TIMEOUT_HALF) -> Check last receive
...
void * S_thread() {
    ...
    while(1) {
        usleep(TIMEOUT_HALF);
        for(int i=0; i<MAX_SOCKET_COUNT; i++) {
            if (addr[i].isfree == 0 && (addr[i].swnd.sb_end != addr[i].swnd.sb_start) && check_send_timeout(&addr[i].last_send)) {
                gettimeofday(&addr[i].last_send, NULL);
                printf("Message - %s\n", addr[i].send_buffer[0]);
