...
int k_socket(int __domain, int __type, int __protocol) {
    ...
    if (sockid > -1) {
        ...
        bzero(&addr[sockid].source, sizeof(struct sockaddr_in));
        bzero(&addr[sockid].destination, sizeof(struct sockaddr_in));
...
int k_recvfrom(int sockid, struct sockaddr_in * destination, char * buff) {
    ...
    if (strlen(addr[sockid].receive_buffer) == 0) return -1;
