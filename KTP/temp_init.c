...
int check_send_timeout(struct timeval * last_send, int duration) {
    struct timeval curr;
...
void * R_thread() {
    ...
    while(1) {
        ...
        for(int i=0; i<MAX_SOCKET_COUNT; i++) {
            ...
            } else if (addr[i].isfree && ((addr[i].rwnd.window_size == 0 && strlen(addr[i].receive_buffer) == 0) || 0)) { // check_send_timeout(&addr[i].last_send, 20*TIMEOUT_HALF) -> Check last receive
...
void * S_thread() {
    ...
    while(1) {
        usleep(TIMEOUT_HALF);
        for(int i=0; i<MAX_SOCKET_COUNT; i++) {
            if (addr[i].isfree == 0 && strlen(addr[i].send_buffer) && check_send_timeout(&addr[i].last_send, 2*TIMEOUT_HALF)) {
                ...
                // while(start != end || (start == end && addr[i].swnd.window_size == 0)) {
                //     start = (start + 1)%10;
...
void * Bind_thread() {
    ...
    while(1) {
        ...
        for(int i=0; i<MAX_SOCKET_COUNT; i++) {
            if (addr[i] > 0) {
                char temp[20];
                inet_ntop(AF_INET, &shm_addr[i].source.sin_addr, temp, 20);
                addr[i] = bind(shm_addr[i].sockfd, (struct sockaddr *) &shm_addr[i].source, sizeof(struct sockaddr));
                printf("Request found - %d, %s:%d\n", addr[i], temp, ntohs(shm_addr[i].source.sin_port));
            }
        }
    }
}
